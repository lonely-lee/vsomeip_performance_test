...
latency_measurer::latency_measurer(std::uint32_t _pid) :

    exit_flag_(false) {
}

bool latency_measurer::start() {
    if (clock_gettime(CLOCK_REALTIME, &before_) != 0) {
        return false;
    }
...
void latency_measurer::exit_measure(){
        exit_flag_ = true;
}

double latency_measurer::get_cpu_load_period() const {
        return cpu_load_period_;
}

void latency_measurer::start() {
    // reset everything
...
void latency_measurer::print_cpu_load() const {
...
double latency_measurer::get_cpu_load() const {
    return cpu_load_pid_;
}

std::uint64_t latency_measurer::read_proc_pid_stat() {
...
std::uint64_t latency_measurer::read_proc_stat(std::uint64_t* _idle) {
...
bool latency_measurer::read_clock_ticks() {
